  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 md:p-8 text-center">
        ...
        <p className="text-gray-500 mb-6">Simpan hasil, lihat laporan lengkap, dan lanjutkan eksperimen karirmu setelah login.</p>
        <div className="space-y-3">
          <button onClick={() => router.push("/login")} className="w-full bg-blue-600 text-white py-3 rounded-xl font-medium hover:bg-blue-700 transition">Masuk</button>
          <button onClick={() => router.push("/login")} className="w-full border-2 border-gray-200 text-gray-700 py-3 rounded-xl font-medium hover:bg-gray-50 transition">Daftar Gratis</button>
        </div>
      </div>
    </div>
  );
