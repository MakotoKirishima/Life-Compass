        ...
        if (!r.ok) {
          if (r.status === 401) {
            return fetch(`${api}/api/auth/register`, {
              ...
              if (!r2.ok) throw new Error(d2.detail || "Registrasi gagal");
              return d2;
            });
          }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 flex items-center justify-center px-4 py-8">
      <div className="bg-white p-8 rounded-2xl shadow-lg border max-w-md w-full">
        <a href="/" className="text-blue-600 text-sm mb-6 block hover:underline">← Kembali</a>
        <h1 className="text-2xl font-bold mb-1">Masuk</h1>
        <p className="text-gray-500 mb-8">Lanjutkan perjalanan karirmu</p>

        {error && <div className="bg-red-50 border border-red-200 text-red-600 p-3 rounded-xl mb-4 text-sm">{error}</div>}

        <button onClick={handleGoogleLogin} disabled={loading}
          className="w-full border-2 border-gray-200 py-3 rounded-xl flex items-center justify-center gap-3 hover:bg-gray-50 transition mb-6 disabled:opacity-50">
        ...
        <div className="relative mb-6">
          ...
          <div className="relative flex justify-center"><span className="bg-white px-4 text-gray-400 text-sm">atau</span></div>
        </div>

        <form onSubmit={handleEmailLogin}>
          <label className="block text-sm font-medium text-gray-700 mb-1">Email</label>
          <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="nama@email.com"
            className="w-full border rounded-xl px-4 py-3 mb-4 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none" />
